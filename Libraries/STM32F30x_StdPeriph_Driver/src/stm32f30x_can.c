...
uint8_t CAN_Transmit(CAN_TypeDef * CANx, CanTxMsg * TxMessage)
{
	...
	if (transmit_mailbox != CAN_TxStatus_NoMailBox) {
		...
		CANx->sTxMailBox[transmit_mailbox].TDLR =
		    (((uint32_t) TxMessage->
		      Data[3] << 24) | ((uint32_t) TxMessage->
					Data[2] << 16) | ((uint32_t) TxMessage->
							  Data[1] << 8) |
		     ((uint32_t) TxMessage->Data[0]));
		CANx->sTxMailBox[transmit_mailbox].TDHR =
		    (((uint32_t) TxMessage->
		      Data[7] << 24) | ((uint32_t) TxMessage->
					Data[6] << 16) | ((uint32_t) TxMessage->
							  Data[5] << 8) |
...
uint8_t CAN_OperatingModeRequest(CAN_TypeDef * CANx, uint8_t CAN_OperatingMode)
{
	...
	if (CAN_OperatingMode == CAN_OperatingMode_Initialization) {
		...
		CANx->MCR =
		    (uint32_t) ((CANx->
				 MCR & (uint32_t) (~(uint32_t) CAN_MCR_SLEEP)) |
				CAN_MCR_INRQ);
		...
		CANx->MCR =
		    (uint32_t) ((CANx->
				 MCR & (uint32_t) (~(uint32_t) CAN_MCR_INRQ)) |
				CAN_MCR_SLEEP);
